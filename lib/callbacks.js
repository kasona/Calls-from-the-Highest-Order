...
User.find = function(query, callback){
  ...
  // Step 1 Check if tkeys exist
  if(!hasKeys){
    return callback(new RangeError('Error, matching keys not found'),foundUsers);
  }
  // Step 2 Check if all the type of values match
  var hasTypes = keysInQuery.every(function(key){
    return schema[key] === typeof query[key] ? true : false;
  });
  // Step 3 Check if has same type, if not return error
  if(!hasTypes){
    return callback(new TypeError('Didnt find matching type'), foundUsers);
  }
  // If all passed, compare it to the user and query keys to see if it's the same
  // For each User in ./datastore
  testUsers.forEach(function(user){
    // looking for the keys in the query and matching them to the user
    var matches = keysInQuery.every(function(key){
      // Could use if statement but traverse (?) is faster.
      return user[key] === query[key] ? true : false;
    });
    // If we have things stored in our matches variable,that means we can push our users into foundUsers
    if(matches){
      foundUsers.push(user);
    }
  });
  // Callback error is set to null, so if there is no error then go to foundUsers.
  return callback(null, foundUsers);
};
// Actually using it in a User.find, {id : 2} is the query and function is the callback function ( finds for errors )
